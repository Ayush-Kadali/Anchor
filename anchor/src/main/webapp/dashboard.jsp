    ...
    <div class="main">
        ...
        <div class="sidebar">
            ...
            <div class="section">
                ...
                <div id="activeConnections"><i>Loading...</i></div>
                <br>
...
Or type commands directly in the input box below:
  ...
  status
                ...
                <label style="margin-left:10px;font-size:0.85em;">
                    <input type="checkbox" id="showCommands" checked onchange="toggleEcho()">
                    Show my commands
                </label>
            </div>
    ...
    <script>
        ...
        function toggleEcho() {
            showEcho = document.getElementById('showCommands').checked;
        }
        ...
        function handleTerminalKey(event) {
            if (event.key === 'Enter') {
                ...
                if (ws && ws.readyState === WebSocket.OPEN && cmd.length > 0) {
                    ws.send(cmd);
                    // show what user typed if echo is on
                    if (showEcho) {
                        appendTerminal('> ' + cmd + '\n');
                    }
                }
        ...
        // ==================== ACTIVE CONNECTIONS ====================
        //
        // Polls /api/connections every 3 seconds.
        // Shows active device connections with Join buttons.
        // When you click Join, it connects WebSocket and sends "join <id>".

        function refreshConnections() {
            fetch('api/connections')
                .then(function(response) { return response.json(); })
                .then(function(data) {
                    var div = document.getElementById('activeConnections');
                    if (data.connections.length === 0) {
                        div.innerHTML = '<i>No active connections.</i>';
                        return;
                    }
                    var html = '';
                    for (var i = 0; i < data.connections.length; i++) {
                        ...
                        html += '<div style="padding:6px;margin:4px 0;background:#e8f5e9;border:1px solid #4caf50;">';
                        html += '<b>' + c.type + '</b>: ' + c.description + '<br>';
                        html += '<small>Owner: ' + c.owner + '</small><br>';
                        html += '<button onclick="joinConnection(\'' + c.id + '\')" style="margin-top:4px;">Join</button>';
                        html += '</div>';
                    }
                    div.innerHTML = html;
                })
                .catch(function() {
                    document.getElementById('activeConnections').innerHTML = '<i>Failed to load.</i>';
                });
        }

        function joinConnection(connId) {
            ensureWebSocket(function() {
                ws.send('join ' + connId.substring(0, 8));
            });
        }

        // poll active connections every 3 seconds
        setInterval(refreshConnections, 3000);

        // scan devices and connections on page load
        scanDevices();
        refreshConnections();
    </script>
</body>
